...
function App() {
  ...
  useEffect(() => { const controller = new AbortController(); fetch("/contract.json", { signal: controller.signal }).then((response) => response.ok ? response.json() : Promise.reject()).then((data) => setContract(validateContract(data))).catch(() => undefined); return () => controller.abort(); }, []);
